...
#define TEXT(z, index, container)                    container

#define NAME_ELEMENT(z, index, _)               element ## index

#define EXPECT_COMPARISON(z, index, comp)       EXPECT_EQ(NAME_ELEMENT(_, index, _), std::get<index>(container[loop_index])); \
                                                comp(&NAME_ELEMENT(_, index, _), &std::get<index>(container[loop_index]));
...
#define FOR_LOOP_AUTO_REF_CHECK(cv_type, ref_type, number) \
	for(cv_type auto ref_type [BOOST_PP_ENUM(number, NAME_ELEMENT, _), loop_index] : iterator::with_index(container)) \
	{ \
...
#define DEFINE_VECTOR_TEST(name, description, n) \
	...
	{ \
		std::vector<std::tuple<BOOST_PP_ENUM(n, TEXT, TypeParam)>> container(10); \
